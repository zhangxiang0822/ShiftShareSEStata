...
prog main
	...
	AKM_nocluster, dependant_var(d_sh_empl_mfg) endogenous_var(d_tradeusch_pw) shiftshare_iv(d_tradeotch_pw_lag) ///
...
prog AKM_nocluster
	...
	** IV results withoud AKM adjustment
	...
	if ("`weight_var'" ~= "") {
		ivregress 2sls `dependant_var' `control_varlist' (`endogenous_var' = `shiftshare_iv') [aw = `weight_var']
		local a = e(df_m)
		display `a'
		...
		ivregress 2sls `dependant_var' `control_varlist' (`endogenous_var' = `shiftshare_iv') [aw = `weight_var'], r 
		...
		* ivregress 2sls `dependant_var' `control_varlist' (`endogenous_var' = `shiftshare_iv') [aw = `weight_var'], cluster(state)
		...
		ivregress 2sls `dependant_var' (`endogenous_var' = `shiftshare_iv') [aw = `weight_var']
		local SE_homo = _se[`endogenous_var']
		local t_homo  = _b[`endogenous_var']/_se[`endogenous_var']
		local p_homo  = 2 * ttail(e(df_r),abs(`t_homo'))
		local CI_low_homo = _b[`endogenous_var'] - `critical_value' * `SE_homo'
		...
		ivregress 2sls `dependant_var' (`endogenous_var' = `shiftshare_iv') [aw = `weight_var'], r 
		local SE_r = _se[`endogenous_var']
		local t_r  = _b[`endogenous_var']/_se[`endogenous_var']
		local p_r  = 2 * ttail(e(df_r),abs(`t_r'))
		local CI_low_r = _b[`endogenous_var'] - `critical_value' * `SE_r'
