...
{p2colset 5 18 24 2}{...}
{p2col :{hi: ivreg_ss} {hline 2}}Computes confidence intervals, standard errors, and p-values in a IV regression in which the instrumental variable has a shift-share structure. Several different inference methods can be computed.
{p2colreset}{...}
...
{cmd:,} {opt endogenous_var(varlist)} {opt shiftshare_iv(varlist)} 
		...
		[control_varlist(varlist) weight_var(varlist) akmtype beta0 path_cluster cluster_var(varlist) firststage]
...
{synopt :{opt firststage}}  Specifying whether to show first-stage results or not. It must take value 0 or 1. If it equals 1, the first-stage results and inference are reported. If you want to see first-stage results, you should also install "reg_ss" command. The default value is 0. {p_end}
