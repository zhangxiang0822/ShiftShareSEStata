...
{synopt :{opt akmtype}} Specifies which inference method to use. It must equal 0 or 1. If it equals 1, "ivreg_ss" applies the AKM inference procedure described in 
Adão, Kolesár, and Morales (2019). If it equals 0, "ivreg_ss" applies the AKM0 inference procedure described in Adão, Kolesár, and Morales (2019). If it equals 0, the 
...
{synopt :{opt firststage}} If it equals 1, the first-stage results are reported, otherwise only second-stage results are reported. If it equals 1, the reg_ss command must be previously
							installed. The default value is 0. {p_end}
...
the ivreg_ss function will return an error message "Share matrix is collinear".
...
, this has to be done before using the ivreg_ss function.{p_end}

{marker Description}{...}
{title:Note on Dataset Preparation}
{pstd} Before running regressions using ivreg_ss, please make sure there are no missing values in all variables that you pass to the program. {p_end}
