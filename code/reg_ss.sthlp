...
{p2colset 5 18 24 2}{...}
{p2col :{hi: reg_ss} {hline 2}}Inference in OLS regressions with shift-share structure. See also ivreg_ss for Inference in IV regression with shift-share structure. 
{p2colreset}{...}
...
{synoptline}
{syntab:Model}
{p2coldent:* {opt shiftshare_var(varlist)}} Shift-share variable {p_end}
...
{marker discription}{...}
{title:Description}
{pstd} {opt reg_ss} Provides standard error estimation and confidence intervals in instrumental variables regressions when the instrument has a shift-share structure.{p_end}
...
{phang2}{cmd:.	reg_ss d_sh_empl_mfg, endogenous_var(d_tradeusch_pw) shiftshare_iv(d_tradeotch_pw_lag) control_varlist(`control_varlist') share_varlist(emp_share*) weight_var(weight) akmtype(0) path_cluster(`local_path') cluster_var(sec_3d)}

For more examples, please see "https://github.com/zhangxiang0822/BartikSEStata/blob/master/code/ADHapplication.do".

{marker author}{...}
{title:Bug Reporting}
{pstd} reg_ss is part of an ongoing project and thus may contain errors and malfunctions. Pleas submit bugs, comments, and suggestions to the Github repository at "https://github.com/zhangxiang0822/BartikSEStata". You're free to open an issue. 
